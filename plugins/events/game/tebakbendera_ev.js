neoxr.create(async (m, {
   ...
   try {
      ...
      if (m.quoted && /Codename5: guess flag/i.test(m.quoted.text)) {
         if (!(id in client.tebakbendera) && /Codename5: guess flag/i.test(m.quoted.text)) return client.reply(m.chat, Func.texted('bold', `🚩 Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}tebakbendera_ untuk mendapatkan soal baru.`), m)
         if (m.quoted.id == client.tebakbendera[id][0].id) {
            let json = JSON.parse(JSON.stringify(client.tebakbendera[id][1]))
            ...
            if (body.toLowerCase() == json.name.toLowerCase()) {
               await client.sendSticker(m.chat, await Func.fetchBuffer('./media/sticker/true.webp'), m, {
                  ...
                  users.point += reward
                  clearTimeout(client.tebakbendera[id][2])
