neoxr.create(async (m, {
   ...
   try {
      ...
      if (m.quoted && /Codename1: arrange words/i.test(m.quoted.text)) {
         if (!(id in client.susunkata) && /Codename1: arrange words/i.test(m.quoted.text)) return client.reply(m.chat, Func.texted('bold', `🚩 Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}susunkata_ untuk mendapatkan soal baru.`), m)
         if (m.quoted.id == client.susunkata[id][0].id) {
            let json = JSON.parse(JSON.stringify(client.susunkata[id][1]))
            ...
            if (body.toLowerCase() == json.jawaban.toLowerCase()) {
               await client.sendSticker(m.chat, await Func.fetchBuffer('./media/sticker/true.webp'), m, {
                  ...
                  users.point += reward
                  clearTimeout(client.susunkata[id][2])
