neoxr.create(async (m, {
   ...
   try {
      ...
      if (m.quoted && /Codename2: Brain teaser/i.test(m.quoted.text)) {
         if (!(id in client.asahotak) && /Codename2: Brain teaser/i.test(m.quoted.text)) return client.reply(m.chat, Func.texted('bold', `🚩 Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}asahotak_ untuk mendapatkan soal baru.`), m)
         if (m.quoted.id == client.asahotak[id][0].id) {
            let json = JSON.parse(JSON.stringify(client.asahotak[id][1]))
            ...
            if (body.toLowerCase() == json.jawaban.toLowerCase()) {
               await client.sendSticker(m.chat, await Func.fetchBuffer('./media/sticker/true.webp'), m, {
                  ...
                  users.point += reward
                  clearTimeout(client.asahotak[id][2])
